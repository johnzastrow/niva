...
BUILTINS = [
    (
        "load",
        '`load <file>` · `load "<file>\\|layername=<layer>"` · `load @conn[.schema].table`',
    ...
    (
        "describe",
        "`describe <verb-or-algorithm-id>` (CLI/API, not a flow stage)",
...
EXAMPLES = [
    (
        "native:convexhull",
        "Convex hull around one layer of a multi-layer GeoPackage",
    ...
    (
        "native:slope",
        "Slope raster from a DEM — raster output, so give an explicit OUTPUT path",
        "run native:slope INPUT=dem.tif Z_FACTOR=1 NODATA=-9999 OUTPUT=slope.tif",
    ),
    (
        "native:countpointsinpolygon",
        "Count points in each polygon — two explicit inputs, no pipe",
...
def main():
    ...
    for verb in reg.verbs():
        ...
        L.append(
            f"| `{verb}` | `{alias.algorithm}` | {niva_sig(alias)} | ✅ | {orig_sig(info)} |"
        )
    L += [
        "",
