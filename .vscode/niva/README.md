...
- **Tab-completion snippets** — type a prefix and Tab to expand with placeholders for each parameter. Covers all 21 built-ins, all 45 aliases, common pipeline patterns (`buf`, `pipeline`, `each-reproject`, `sql-pipe`, …), and the **PDAL/SAGA LiDAR harness** (`pdalcli`, `pdalcli-dtm`, `pdalcli-dsm`, `pdalcli-class`, `chm`, `saga`)
