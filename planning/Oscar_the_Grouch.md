...
| U12 | **`add` is session-dependent.** A flow ending in `add` (no `save`) produces a layer interactively but **nothing headless** — the same `.niva` behaves differently — surfaced by `13-§11`. | 🟨 | Headless `add` warns/skips; an add-only flow errors headless (`03-§2.7`); teach `save` for persistence. |
