...
| `niva lsp` | **Language Server** over stdio for `.niva` files — completion (verbs → options → enums → paths), live diagnostics (the offline validator), and hover (`describe`) in any LSP editor (Neovim, Helix, VS Code, Kate). Same engine as the repl (`niva.intelligence`); stdlib JSON-RPC, no extra deps. See [editor integration](editor-integration.md#language-server--niva-lsp-real-completion-diagnostics-hover) |
