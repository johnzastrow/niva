...
| **Credential exposure** | Database logins live in **QGIS's connection store**, referenced only by `@name` (`02-§3.5`). niva **never stores, logs, or transmits credentials**; the op-log and lineage **redact** connection secrets/tokens. The **write** path (`save @conn.table`, non-SELECT `sql @conn`, v0.17.0) preserves this end-to-end: the destination URI is built from the **live QGIS connection** (host/db/login resolved by QGIS), so no credential — or query/URI text — appears in the flow or any error message. |
...
- **No silent overwrite of an input** (`03-§2.5`) — and **`save @conn.table` is
  fail-closed**: it errors if the table exists unless `mode=replace`/`mode=append`.
