...
**Bundled example + authoring guide** (v0.27.0). niva ships a fully-populated `example`
template under `niva/templates/` (regenerated by `templates/build_example.py`): three styled
slots (`boundary`/`roads`/`places`), a print layout, a bookmark, a title + CRS. The resolver's
third tier finds it, so `from-template=example` works with no setup; the placeholder data is
stored with **relative** paths so the committed template is self-contained, and it ships in
