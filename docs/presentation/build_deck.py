...
def _footer(slide, n):
    ...
    _text(
        ...
        [
            [
                ("niva", {"bold": True, "color": DEEP}),
                ("  ·  Easy wins every time", {"color": DEEP}),
            ]
        ],
        size=12,
    )
...
def content_slide(prs, title, eyebrow=None):
    ...
    ty = 0.55
    if eyebrow:
        _text(s, 0.8, 0.5, 11.8, 0.35, eyebrow.upper(), size=12.5, color=TAN, bold=True)
        ty = 0.82
...
def fit_image(slide, path, x, y, w, h, align="center"):
    ...
    slide.shapes.add_picture(
        path, Inches(dx), Inches(dy), Inches(draw_w), Inches(draw_h)
    )
...
s.shapes.add_picture(
    logo, Inches((SW - disp_w) / 2), Inches(0.7), Inches(disp_w), Inches(disp_h)
)
_text(
    s,
    1,
    ...
    [
        [
            ...
            (
                ...
                "that they write code. So the work waits, or doesn't happen.",
                {"size": 20, "color": SAGE},
            ),
        ]
    ],
    ...
    [
        [
            ...
            (
                ...
                "and tedious even for those who can.",
                {"size": 19, "color": INK},
            ),
        ]
    ],
...
_text(
    ...
    [
        [
            ("load", {"color": TAN, "bold": True}),
            (" roads.gpkg ", {"color": WHITE}),
            ("| ", {"color": MIST}),
            ("buffer", {"color": TAN, "bold": True}),
            (" 100 dissolve ", {"color": WHITE}),
            ("| ", {"color": MIST}),
            ("clip", {"color": TAN, "bold": True}),
            (" city.gpkg ", {"color": WHITE}),
            ("| ", {"color": MIST}),
            ("save", {"color": TAN, "bold": True}),
            (" roads_local.gpkg", {"color": WHITE}),
        ]
    ],
    size=19,
    font="Consolas",
...
_text(
    ...
    [
        [
            (
                "A whole pipeline on one line — that a non-programmer can ",
                {"size": 20, "color": SAGE},
            ),
...
for i, (h, body, accent) in enumerate(cards):
    ...
    _text(
        s, x + 0.25, 3.75, cw - 0.5, 2.4, body, size=16, color=SAGE, line_spacing=1.15
    )
_footer(s, 6)
...
s = bullets_slide(
    ...
    eyebrow="Marimo-native",
    lead="Marimo is how your team makes analysis repeatable and shareable. niva runs right inside it:",
    items=[
        (
            "Write niva in a Marimo cell",
            "a readable pipeline beside your Python and charts",
        ),
...
_text(
    ...
    [
        [
            ...
            (
                "niva meets your people where they already work.",
                {"size": 18, "color": DEEP, "bold": True},
            ),
        ]
    ],
)
...
image_slide(
    prs, "One grammar, many surfaces", "surfaces.png", eyebrow="Flexible by design"
)
...
s = bullets_slide(
    ...
    items=[
        (
            "Pipelines are the record",
            "the one-line flow (or YAML) IS the documentation",
        ),
...
for i, (num, lab) in enumerate(stats):
    ...
    _fill(card, LIGHT)
    _text(
        s, x, 2.35, cw, 1.2, num, size=58, color=DEEP, bold=True, align=PP_ALIGN.CENTER
    )
...
s = bullets_slide(
    prs,
    "Why niva, why now",
    eyebrow="The tailwinds",
    items=[
        (
            "QGIS is the de-facto open GIS",
            "ubiquitous, trusted, free — the foundation we build on",
        ),
        (
            "Reproducible analysis is the norm",
            "Marimo and notebooks are how modern teams work",
        ),
        (
            "The automation gap is real & unserved",
            "GUI analysts still can't automate without code",
        ),
        (
            "Open and clean-room",
            "GPLv3, built on PyQGIS; no proprietary lock-in to fear",
        ),
    ],
)
_footer(s, 14)
...
for i, (ctitle, accent, items) in enumerate(status_cards):
    ...
    paras = [
        [
            ("✓  ", {"color": DEEP, "bold": True, "size": 14}),
            (it, {"color": SAGE, "size": 14}),
        ]
        for it in items
    ]
...
for ver, mtitle, sub, tag in milestones:
    ...
    if tag:
        head[0].append(
            ("   " + tag, {"size": 13, "color": PAIN, "bold": True, "italic": True})
        )
...
mw_in = mh_in * mw / mh
s.shapes.add_picture(
    mark, Inches((SW - mw_in) / 2), Inches(1.15), Inches(mw_in), Inches(mh_in)
)
_text(
    s,
    1,
